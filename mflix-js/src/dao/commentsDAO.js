  ...
  */
  ...
  static async addComment(movieId, user, comment, date) {
    try {
      ...
      const commentDoc = { name:user.name, email:user.email, movie_id: new ObjectId(movieId), text:comment, date:date }
  ...
  static async mostActiveCommenters() {
    ...
    */
    try {
      ...
      const readConcern = {level:'majority'} //alcance de replica
